...
const CheckOut = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const userMyInfo = await profileService.getMyInfo();
      const userId = userMyInfo.data.result.profileId;
              ...
              <TableBody>
                {selectedProducts.map((product) => (
                  <TableRow key={product.cartItemId}>
                    <TableCell sx={{ color: '#e0e0e0' }}>{product.productId.name}</TableCell>
                    <TableCell align="right" sx={{ color: '#e0e0e0' }}>{product.productId.price} VND</TableCell>
                    <TableCell align="right" sx={{ color: '#e0e0e0' }}>{product.quantity}</TableCell>
                    <TableCell align="right" sx={{ color: '#e0e0e0' }}>{product.productId.price * product.quantity} VND</TableCell>
                  </TableRow>
                ))}
                <TableRow>
